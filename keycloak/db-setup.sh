...
Create the Keycloak PostgreSQL role and database on the shared postgres
instance (postgres/). Run this BEFORE deploying Keycloak
(kubectl apply -k keycloak/).

The role password is auto-detected from keycloak/.env (KC_DB_PASSWORD);
override with --db-pass if needed.

Options:
  --db-pass PASSWORD   Keycloak DB role password (auto-detected from .env)
...
# Resolve Keycloak DB password from .env when not provided.
if [[ -z "$DB_PASS" ]]; then
    if [[ -f "${SCRIPT_DIR}/.env" ]]; then
        DB_PASS="$(awk -F= '/^KC_DB_PASSWORD=/ {
            sub(/^KC_DB_PASSWORD=/, "", $0)
            gsub(/[[:space:]]/, "", $0)
            print
            exit
        }' "${SCRIPT_DIR}/.env")"
        if [[ -n "$DB_PASS" ]]; then
            echo "Using Keycloak DB password from .env"
        else
            echo "Error: no KC_DB_PASSWORD found in .env." >&2
            echo "  Copy .env.example to .env and fill it in first." >&2
            exit 1
        fi
    else
        echo "Error: --db-pass is required when .env does not exist." >&2
...
if [[ "$DRY_RUN" == true ]]; then
    echo "DRY-RUN: would create role 'keycloak' and database keycloak in postgres"
    exit 0
fi

echo "Creating Keycloak role and database in the shared postgres..."
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'keycloak') THEN
        CREATE ROLE keycloak WITH LOGIN PASSWORD '${DB_PASS}';
        RAISE NOTICE 'Role keycloak created';
    ELSE
        RAISE NOTICE 'Role keycloak already exists';
    END IF;
END
\$\$;
SQL
